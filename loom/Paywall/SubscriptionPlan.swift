...
enum SubscriptionPlan: String, CaseIterable, Identifiable {
    ...
    var trialDetailText: LocalizedStringKey? {
        ...
        case .annual:
            ...
            let remainingDays = max(0, calendar.dateComponents([.day], from: today, to: endDay).day ?? 0)
            let dayWord = remainingDays == 1 ? "day" : "days"
            return LocalizedStringKey("Start in \(remainingDays) \(dayWord) to get locked annual pricing for life")
        case .monthly:
            return nil
        }
    }
    ...
    var disclosureDetailText: LocalizedStringKey? {
        switch self {
        case .lifetime:
            return "Lifetime - Founding Member access is a one-time payment. Members enjoy all Loom features for life and get exclusive, early access to innovations. This offer is limited to the first 1,000 members and ends May 31, 2026."
        case .annual:
            return "Annual includes a 10-day free trial. Payment will be charged to your Apple ID at the end of the trial period unless canceled at least 24 hours before the trial ends. Subscription renews automatically unless canceled at least 24 hours before the end of the current period. Your account will be charged for renewal within 24 hours prior to the end of the current period. You can manage or cancel your subscription anytime in your Apple ID account settings."
        case .monthly:
